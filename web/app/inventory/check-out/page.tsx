...
import { Search, PackageMinus, Loader2, CheckCircle, AlertCircle, Package, AlertTriangle } from "lucide-react"
...
export default function CheckOutPage() {
	...
	const handleCheckOut = async () => {
		...
		try {
			...
			if (response.ok) {
				setSuccessMessage(`Successfully checked out ${selectedProduct.name}`)
				...
				setErrorMessage(error.error || 'Failed to check out product')
			}
		} catch (error) {
			console.error('Error checking out product:', error)
			setErrorMessage('An error occurred while checking out the product')
		} finally {
	...
	const openConfirmDialog = (product: Product) => {
		if (product.quantity <= 0) {
			setErrorMessage('Cannot check out product with zero quantity')
			...
			<main className="flex-1 md:ml-64 p-6">
				<div className="max-w-7xl mx-auto space-y-6">
					...
					<div className="flex items-center justify-between">
						<div>
							<h1 className="text-3xl font-bold tracking-tight">Check Out Inventory</h1>
					...
					<Card>
						<CardHeader>
							<CardTitle>Select Product to Check Out</CardTitle>
							<CardDescription>
								Choose a product from the list below to remove from inventory
							</CardDescription>
						</CardHeader>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								{filteredProducts.length} product(s) available for checkout
							</CardDescription>
						</CardHeader>
						<CardContent>
							{isLoading ? (
								...
								<div className="text-center py-8 text-muted-foreground">
									...
									<p className="text-sm mt-2">All products are out of stock or filtered out</p>
								</div>
							) : (
								<Table>
									<TableHeader>
											...
											<TableRow key={product.product_id}>
												...
												<TableCell className="text-right">
													<Button
														...
														disabled={isCheckingOut || product.quantity <= 0}
													>
			...
			<Dialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Are you sure you want to check out this product? This will reduce the inventory.
						</DialogDescription>
					</DialogHeader>
					{selectedProduct && (
						<div className="space-y-4 py-4">
							...
							<div className="space-y-2">
								...
								<p className="text-sm text-red-600 font-semibold">{selectedProduct.quantity - 1}</p>
							</div>
							{selectedProduct.quantity - 1 < 10 && (
								<div className="flex items-center gap-2 p-3 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-md">
									...
									<span className="text-sm text-orange-800 dark:text-orange-200">
										Warning: Stock will be low after this checkout
									</span>
								</div>
							)}
						</div>
